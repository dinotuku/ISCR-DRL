...
class SimulatedUser(object):
    ...
    def __call__(self, query, ans, ans_index):
        ...
        self.keytermlist = readKeytermlist(keyterm_dir, query)
        self.requestlist = readRequestlist(request_dir, self.ans)
        self.topiclist = readTopicWords(topic_dir)
        # Since top ranking is pre-sorted according to query
        self.topicRanking = readTopicList(ranking_dir, ans_index)[:5]
    ...
    def feedback(self, request):
        ...
        if action == 'firstpass':
            ...
            if self.use_survey:
                l = [item[0] for item in ret if item[0] in self.ans]
                ...
                doc = next((item[0] for item in ret if item[0] in self.ans), None)
            ...
            if len(self.keytermlist) is not 0:
                ...
                cnt = 0.
                for a in self.ans:
                    model_path = os.path.join(self.data_dir, 'docmodel', IndexToDocName(a))
                    if a in readDocModel(model_path):
                        cnt += 1.
