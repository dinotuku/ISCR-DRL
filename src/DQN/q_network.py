...
class DeepQLearner:
    ...
    def __init__(self, input_width, input_height, net_width, net_height, num_actions,
                 ...
                 batch_accumulator, rng, double, dueling, input_scale=255.0):
        ...
        self.double = True if double else False
        self.dueling = True if dueling else False
        ...
        if self.dueling:
            v_vals, a_vals = lasagne.layers.get_output(self.l_out, states / input_scale)
            q_vals = v_vals + a_vals - T.mean(a_vals, axis=1, keepdims=True)
        else:
    ...
    def build_network(self, network_type, input_width, input_height,
                      output_dim, num_frames, batch_size, dueling=False):
    ...
    def build_rl_network_dnn(self, input_width, input_height, output_dim,
        ...
        if dueling:
            v_out = lasagne.layers.DenseLayer(
                layer,
                num_units=1,
                nonlinearity=None,
                W=lasagne.init.Normal(var),
            ...
            a_out = lasagne.layers.DenseLayer(
                layer,
                num_units=output_dim,
                nonlinearity=None,
                W=lasagne.init.Normal(var),
